const Store = (() => {
  const KEYS = {
    ...
    separations: 'hrms_separations',
    separationAnalytics: 'hrms_separation_analytics',
  ...
  function getSeparations() { return load(KEYS.separations, []); }
  function setSeparations(data) { save(KEYS.separations, data); }
  function addSeparation(sep) {
    const list = getSeparations();
    list.unshift(sep);
    save(KEYS.separations, list);
    return sep;
  }
  function updateSeparation(id, updates) {
    save(KEYS.separations, getSeparations().map(s => s.id === id ? { ...s, ...updates } : s));
  }

  function getSeparationAnalytics() { return load(KEYS.separationAnalytics, null); }
  function setSeparationAnalytics(data) { save(KEYS.separationAnalytics, data); }

  function isOwnSeparation(sep, user) {
    if (!sep || !user) return false;
    const sepEmail = (sep.applicantEmail || '').toLowerCase().trim();
    const userEmail = (user.email || '').toLowerCase().trim();
    if (sepEmail && userEmail) return sepEmail === userEmail;
    return sameEmployeeId(sep.employeeId, user.employeeId);
  }

  function normalizeSeparationApprovals(approvals) {
    const plain = toPlainApprovals(approvals);
    return {
      reporting_manager: plain.reporting_manager ?? plain.manager ?? 'Pending',
      hr: plain.hr ?? 'Pending',
  ...
  function getActiveSeparationSteps(approvals) {
    const a = normalizeSeparationApprovals(approvals);
    return ['reporting_manager', 'hr', 'admin'].filter(key => a[key] !== 'N/A');
  }

  function canApproveSeparationStep(approvals, roleKey, sep) {
  ...
  function canUserApproveSeparation(user, sep) {
    ...
    if (role !== 'admin') {
      ...
      return canApproveSeparationStep(sep.approvals, roleToApprovalKey(role), sep);
    }

    const a = normalizeSeparationApprovals(sep.approvals);
    const active = getActiveSeparationSteps(a);
    return active.some((key, idx) => {
      if (a[key] !== 'Pending') return false;
      for (let i = 0; i < idx; i++) {
        if (a[active[i]] !== 'Approved') return false;
      }
      if (key === 'reporting_manager' && sep.managerReview?.status !== 'Approved') return false;
      return true;
    });
  }

  const SEPARATION_APPROVAL_STEPS = [
    { key: 'reporting_manager', label: 'Reporting Manager' },
    { key: 'hr', label: 'HR' },
    { key: 'admin', label: 'Admin' }
  ];
  ...
  function isExitStepAvailable(sep, stepKey, user) {
    if (!sep || !canRoleUpdateExitStep(user, stepKey)) return false;
    if (!['Approved', 'In_Exit'].includes(sep.status)) return false;
    const approvals = normalizeSeparationApprovals(sep.approvals);
    if (approvals.reporting_manager !== 'Approved') return false;

    const steps = sep.exitSteps || {};
    const done = key => steps[key]?.status === 'Completed';
    if (stepKey === 'knowledge_transfer') return true;
    if (!done('knowledge_transfer')) return false;
    if (stepKey === 'manager_clearance') return true;
    ...
    if (clearanceSteps.includes(stepKey)) return true;
    if (stepKey === 'final_settlement') return clearanceSteps.every(done);
    if (stepKey === 'deactivation') return done('final_settlement');
    return false;
  }

 function getAvailableExitSteps(sep, user) {
  return EXIT_STEP_ORDER.map((key) => ({
    key,
    label: EXIT_STEP_LABELS[key],
    available: isExitStepAvailable(sep, key, user),
    status: sep.exitSteps?.[key]?.status || 'Pending'
  ...
  return {
    ...
    getPerformance, setPerformance, addPerformance, updatePerformance, getPerfTemplates, setPerfTemplates,
    ...
    getSeparations, setSeparations, addSeparation, updateSeparation,
    getSeparationAnalytics, setSeparationAnalytics,
    isOwnSeparation, normalizeSeparationApprovals, getActiveSeparationSteps,
    canApproveSeparationStep, canUserApproveSeparation, canRoleUpdateExitStep,
    isExitStepAvailable, getAvailableExitSteps, SEPARATION_APPROVAL_STEPS, EXIT_STEP_LABELS,
