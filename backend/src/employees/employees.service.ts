...
type MutableEmployeeDto = CreateEmployeeDto & Record<string, unknown>;

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

function parseJsonField(field: string, value: unknown) {
...
function normalizeObjectField(parsed: Record<string, unknown>, field: string) {
  const value = parseJsonField(field, parsed[field]);
  if (value === undefined) {
    delete parsed[field];
    return;
  }
...
function normalizeArrayField(parsed: Record<string, unknown>, field: string) {
  const value = parseJsonField(field, parsed[field]);
  if (value === undefined) {
    delete parsed[field];
    return;
...
function parseEmployeeBody(body: Record<string, unknown> | CreateEmployeeDto): CreateEmployeeDto {
  ...
  for (const field of ['address', 'passport', 'bankDetails']) normalizeObjectField(parsed, field);
  for (const field of ['education', 'experience']) normalizeArrayField(parsed, field);
...
export class EmployeesService {
  ...
  private async assertUniqueEmployeeFields(dto: CreateEmployeeDto, excludeId?: string) {
    const employeeId = trimmedString(dto.employeeId)?.toUpperCase();
    const email = trimmedString(dto.email)?.toLowerCase();
    const officeMail = trimmedString(dto.officeMail)?.toLowerCase();
    const mobile = trimmedString(dto.mobile);
    const aadhaar = trimmedString(dto.aadhaar);
    const pan = trimmedString(dto.pan)?.toUpperCase();
    const passportNumber = trimmedString(dto.passport?.number);
    const bankAccount = trimmedString(dto.bankDetails?.accountNumber);
    if (officeMail) {
  ...
  async create(
    ...
    const emp = await this.model.create({
      ...dto,
      employeeId: trimmedString(dto.employeeId)?.toUpperCase(),
      email: trimmedString(dto.email)?.toLowerCase(),
      officeMail: trimmedString(dto.officeMail)?.toLowerCase(),
      pan: trimmedString(dto.pan)?.toUpperCase(),
      idDocuments,
      educationDocuments,
      status: 'Active',
    });
  ...
  async update(
    ...
    const dto = parseEmployeeBody(body) as MutableEmployeeDto;
    validateDobIsAdult(dto.dob);
    ...
    if (bankDetails === undefined ||
      (bankDetails &&
        !trimmedString(bankDetails.bankName) &&
        !trimmedString(bankDetails.accountNumber) &&
        !trimmedString(bankDetails.ifscCode))) {
    ...
    if (dto.email) update.email = trimmedString(dto.email)?.toLowerCase();
    if (dto.officeMail) update.officeMail = trimmedString(dto.officeMail)?.toLowerCase();
    if (dto.pan) update.pan = trimmedString(dto.pan)?.toUpperCase();
  ...
  async downloadDocument(id: string, type: 'id' | 'education', index: number, res: Response) {
    if (!['id', 'education'].includes(type)) {
      throw new BadRequestException('Document type must be id or education');
    }
    if (index < 0) {
      throw new BadRequestException('Document index must be zero or greater');
    }
