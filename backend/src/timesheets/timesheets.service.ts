...
export class TimesheetsService {
  ...
  async getTeamTimesheets(managerEmployeeId: string) {
    ...
    const directReportIds = directReports.map((emp) => emp.employeeId).filter(Boolean);
    ...
    const timesheetByEmployeeId = new Map(timesheets.map((ts) => [this.normalizeEmployeeId(ts.employeeId), ts]));
    return directReports.map((employee) => {
      const employeeId = this.normalizeEmployeeId(employee.employeeId);
      const timesheet = timesheetByEmployeeId.get(employeeId);
      const overview = {
        id: null,
        employeeId: employee.employeeId || employeeId,
        employeeName: employee.name || employee.officeMail || employee.email || employeeId,
        department: employee.department || '',
        email: employee.officeMail || employee.email || '',
        date: today,
        status: 'Not Punched In',
        punchInTime: null,
        punchOutTime: null,
      ...
      return {
        ...overview,
        ...mapDoc(timesheet),
        employeeId: employee.employeeId || timesheet.employeeId,
        employeeName: employee.name || timesheet.employeeName || employeeId,
        department: employee.department || '',
        email: employee.officeMail || employee.email || timesheet.email || '',
      };
    });
  }
  ...
  async getAllEmployeeTimesheets() {
    const today = this.getTodayDate();
    ...
    const employeeMap = new Map(activeEmployees.map((emp) => [this.normalizeEmployeeId(emp.employeeId), emp]));
    const timesheetByEmployeeId = new Map(timesheets.map((ts) => [this.normalizeEmployeeId(ts.employeeId), ts]));

    const result = employeeIds.map((empId) => {
      const normalizedEmpId = this.normalizeEmployeeId(empId);
      const timesheet = timesheetByEmployeeId.get(normalizedEmpId);
      const employee = employeeMap.get(normalizedEmpId);

      const overview = {
        _id: null,
        employeeId: empId,
        employeeName: employee?.name || employee?.officeMail || employee?.email || empId,
        department: employee?.department || '',
        email: employee?.officeMail || employee?.email || '',
        date: today,
      ...
      return {
        ...
        ...mapDoc(timesheet),
        employeeId: empId,
        employeeName: employee?.name || timesheet.employeeName || empId,
        department: employee?.department || '',
        email: employee?.officeMail || employee?.email || timesheet.email || '',
      };
    });
