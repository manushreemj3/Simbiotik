...
export class TimesheetsController {
  ...
  async getMyHistory(@Req() req: any, @Query('limit') limitStr?: string) {
    ...
    return Promise.all(timesheets.map((ts) => this.timesheets.enrichTimesheetWithCalculations(ts)));
  }
  ...
  async getEmployeeHistory(
    ...
    return {
      ...result,
      data: await Promise.all(result.data.map((ts) => this.timesheets.enrichTimesheetWithCalculations(ts))),
    };
  }
